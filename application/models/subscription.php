...
class Subscription extends CI_Model {
    ...
    function getByDate($start, $end) {
        $sql = 'SELECT s.* , st.description AS subscription_type, c.client_id as client_id, c.name AS name, c.surname AS surname, c.ci AS ci FROM subscriptions s, subscription_types st, clients c WHERE s.subscription_types_subscription_type_id = st.subscription_type_id AND s.clients_client_id = c.client_id AND s.start_date BETWEEN "' . $start . '" AND "' . $end . '"';
        $query = $this->db->query($sql);
        return $query->result();
    }

    function earnings($start, $end) {

        $sql = 'SELECT SUM( price ) AS count FROM subscriptions WHERE start_date BETWEEN "' . $start . '" AND  "' . $end . '" AND paid =1';
        $query = $this->db->query($sql);
        $count = $query->result_array();
    ...
    function earningsList($start, $end) {
